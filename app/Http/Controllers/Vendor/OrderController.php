...
class OrderController extends Controller
{
    ...
    /**
     * Generate & download today's orders report for this vendor.
     */
    public function downloadTodayReport()
    {
        ...
        $q = DB::table('orders')->whereDate('created_at', $today);
        ...
        if (Schema::hasColumn('orders', 'vendor_id')) {
            ...
            $q->whereExists(function ($sub) use ($vendorId) {
                $sub->select(DB::raw(1))
                    ->from('order_items')
                    ->join('products', 'order_items.product_id', '=', 'products.id')
                    ->whereColumn('order_items.order_id', 'orders.id')
        ...
        if ($orders->isEmpty()) {
            return redirect()->back()->with('error', 'No orders found for today.');
        }

        $pdf = Pdf::loadView('vendor_panel.reports.today_orders', [
            'orders' => $orders,
            'date' => $today->format('Y-m-d')
        ]);

        return $pdf->download('today_orders_' . $today->format('Y-m-d') . '.pdf');
    }
}
