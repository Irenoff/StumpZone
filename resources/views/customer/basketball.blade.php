...
@section('content')
    <!-- Gradient Background -->
    <div class="fixed top-0 left-0 w-full h-full -z-10 bg-gradient-to-br from-gray-900 via-purple-900 to-gray-800"></div>
    ...
    <div class="relative z-10">
        ...
        <div class="mb-12 text-center">
            <h2 class="mb-3 text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-500 md:text-5xl animate-fade-in-down">
                <i class="mr-2 fas fa-basketball-ball"></i>
                Basketball Equipment
            </h2>
            <p class="text-lg text-purple-300 animate-fade-in-up">Professional gear for court champions</p>
            <div class="w-24 h-1 mx-auto mt-4 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 animate-grow-x"></div>
        </div>

        <!-- Equipment Grid -->
        <div class="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            @foreach ($items as $item)
                <div class="flex flex-col h-full overflow-hidden transition-all duration-500 transform bg-gray-800 border border-gray-700 shadow-xl rounded-xl hover:shadow-2xl hover:-translate-y-3 group backdrop-blur-sm bg-opacity-70 hover:border-purple-500">
                    ...
                    <div class="flex flex-col flex-grow p-6">
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <span class="px-2 py-1 text-xs font-semibold 
                                      @if($item->status === 'available') text-purple-200 bg-purple-900
                        ...
                        <p class="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400">Rs {{ $item->price }}</p>
                        ...
                        <div class="pt-4 mt-auto">
                            @if($item->status === 'available' && $item->quantity > 0)
                                ...
                                @else
                                    <form method="POST" action="{{ route('cart.add') }}">
                                        ...
                                        <input type="hidden" name="sport_type" value="basketball">
                                        <button type="submit"
                                                class="w-full px-6 py-3 text-sm font-medium text-white transition-all duration-300 transform bg-gradient-to-r from-purple-600 to-pink-600 rounded-xl hover:from-purple-700 hover:to-pink-700 hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 focus:ring-offset-gray-800 shadow-lg hover:shadow-purple-500/20">
                            ...
                            @elseif($item->status === 'pre_order')
                               @if($item->in_cart)
                                ...
                                @endif
                            @else
                                <div class="w-full px-6 py-3 text-center text-pink-200 bg-pink-900 border border-pink-800 bg-opacity-30 rounded-xl">
                        ...
                        <div class="w-full mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                ...
                                <div class="flex items-center justify-between mb-6">
                                    <span id="modalPrice" class="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400"></span>
        ...
        function showProductDetails(item) {
            ...
            if (item.size && item.size.trim().length > 0) {
                ...
                if (sizes.length > 0) {
                    sizes.forEach(size => {
                        ...
                        sizeElement.className = 'px-3 py-1 text-sm font-medium text-white bg-gray-700 rounded-full cursor-pointer hover:bg-purple-600 size-option';
                    ...
                    if (sizes[0]) {
                        ...
                        sizeContainer.firstChild.classList.add('bg-purple-600');
                    }
                } else {
            ...
            if (item.status === 'pre_order') {
                ...
                if (item.in_cart) {
                    ...
                    cartButtonContainer.innerHTML = `
                        <button type="submit" class="px-6 py-2 text-sm font-medium text-white transition-all duration-300 transform bg-gradient-to-r from-purple-600 to-pink-600 rounded-lg hover:from-purple-700 hover:to-pink-700 hover:scale-[1.02]">
                            <span class="flex items-center">
                                <i class="mr-2 fas fa-shopping-cart"></i>
                                Add to Cart
                            </span>
