...
@section('content')
    <!-- Gradient Background -->
    <div class="fixed top-0 left-0 w-full h-full -z-10 bg-gradient-to-br from-gray-900 via-green-900 to-gray-800"></div>
    ...
    <div class="relative z-10">
        ...
        <div class="mb-12 text-center">
            <h2 class="mb-3 text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-emerald-500 md:text-5xl animate-fade-in-down">
                <i class="mr-2 fas fa-baseball-ball"></i>
                Cricket Equipment
            </h2>
            <p class="text-lg text-green-300 animate-fade-in-up">Professional gear for cricket champions</p>
            <div class="w-24 h-1 mx-auto mt-4 rounded-full bg-gradient-to-r from-green-500 to-emerald-500 animate-grow-x"></div>
        </div>

        <!-- Equipment Grid -->
        <div class="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            @foreach ($items as $item)
                <div class="flex flex-col h-full overflow-hidden transition-all duration-500 transform bg-gray-800 border border-gray-700 shadow-xl rounded-xl hover:shadow-2xl hover:-translate-y-3 group backdrop-blur-sm bg-opacity-70 hover:border-green-500">
                    ...
                    <div class="relative overflow-hidden cursor-pointer" onclick="showProductDetails({{ json_encode([
                        ...
                        <span class="absolute top-3 right-3 px-3 py-1 text-xs font-bold text-white rounded-full shadow-lg 
                            ...
                            @else {{ $item->quantity }} in stock @endif
                        </span>
                    </div>
                    ...
                    <div class="flex flex-col flex-grow p-6">
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <span class="px-2 py-1 text-xs font-semibold 
                                      @if($item->status === 'available') text-green-200 bg-green-900
                        ...
                        <p class="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-emerald-400">Rs {{ $item->price }}</p>
                        
                        <!-- Add to Cart Button Container (Fixed Position) -->
                        <div class="pt-4 mt-auto">
                            @if($item->status === 'available' && $item->quantity > 0)
                                ...
                                @else
                                    <form method="POST" action="{{ route('cart.add') }}">
                                        ...
                                        <input type="hidden" name="sport_type" value="cricket">
                                        <button type="submit"
                                                class="w-full px-6 py-3 text-sm font-medium text-white transition-all duration-300 transform bg-gradient-to-r from-green-600 to-emerald-600 rounded-xl hover:from-green-700 hover:to-emerald-700 hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 focus:ring-offset-gray-800 shadow-lg hover:shadow-green-500/20">
                            ...
                            @elseif($item->status === 'pre_order')
                                ...
                                @endif
                            @else
                                <div class="w-full px-6 py-3 text-center border text-emerald-200 bg-emerald-900 border-emerald-800 bg-opacity-30 rounded-xl">
                    ...
                    <div class="sm:flex sm:items-start">
                        <div class="w-full mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                ...
                                <!-- Size Information -->
                                <div class="mb-6">
                                    <h4 class="mb-2 text-lg font-semibold text-white"></h4>
                                ...
                                <div class="flex items-center justify-between mb-6">
                                    <span id="modalPrice" class="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-emerald-400"></span>
        ...
        function showProductDetails(item) {
            ...
            if (item.size) {
                ...
                if (sizes.length > 0) {
                    sizes.forEach(size => {
                        ...
                        sizeElement.className = 'px-3 py-1 text-sm font-medium text-white bg-gray-700 rounded-full cursor-pointer hover:bg-green-600 size-option';
                    ...
                    if (sizes[0]) {
                        ...
                        sizeContainer.firstChild.classList.add('bg-green-600');
                    }
                } else {
                    sizeContainer.innerHTML = '<p class="text-gray-400"></p>';
                }
            } else {
                sizeContainer.innerHTML = '<p class="text-gray-400"></p>';
            }
            ...
            if (item.status === 'pre_order') {
                ...
                if (item.in_cart) {
                    ...
                    cartButtonContainer.innerHTML = `
                        <button type="submit" class="px-6 py-2 text-sm font-medium text-white transition-all duration-300 transform bg-gradient-to-r from-green-600 to-emerald-600 rounded-lg hover:from-green-700 hover:to-emerald-700 hover:scale-[1.02]">
                            <span class="flex items-center">
                                <i class="mr-2 fas fa-shopping-cart"></i>
                                Add to Cart
                            </span>
