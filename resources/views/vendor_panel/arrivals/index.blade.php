...
@section('title','Vendor • New Arrivals')

@section('content')
<div class="max-w-6xl px-4 py-10 mx-auto">
    <h1 class="mb-6 text-2xl font-bold text-white">New Arrivals</h1>
    ...
    <div class="p-6 mb-8 border rounded-xl bg-white/5 border-white/10">
        ...
        <form method="POST" action="{{ route('vendor.arrivals.store') }}" enctype="multipart/form-data" class="grid gap-4 md:grid-cols-2">
            @csrf

            <div>
                <label class="block mb-1 text-sm text-slate-200">Title</label>
                <input name="title" value="{{ old('title', old('name')) }}" required
                       class="w-full px-3 py-2 text-white border rounded-lg bg-white/10 border-white/10 placeholder-slate-400">
                @error('title') <div class="mt-1 text-xs text-rose-300">{{ $message }}</div> @enderror
            </div>
            ...
            <div>
                ...
                <select name="sport" required class="w-full px-3 py-2 text-white border rounded-lg bg-white/10 border-white/10">
                    @php $sportOld = old('sport'); @endphp
                    <option value="" disabled {{ $sportOld ? '' : 'selected' }}>Select sport</option>
                ...
                @error('sport') <div class="mt-1 text-xs text-rose-300">{{ $message }}</div> @enderror
            </div>
            ...
            <div class="flex justify-end gap-3 md:col-span-2">
                <button type="submit"
                        class="px-5 py-2 font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700">
                    Save
                </button>
            </div>
        </form>
            ...
            <tbody class="divide-y divide-white/10">
                @forelse($items as $item)
                    <tr class="hover:bg-white/5">
                        ...
                        <td class="px-4 py-3">
                            {{ $item->created_at ? \Illuminate\Support\Carbon::parse($item->created_at)->format('Y-m-d H:i') : '' }}
                        </td>
                        <td class="px-4 py-3 text-right">
                            <a href="{{ route('vendor.arrivals.edit', $item->id) }}"
                               class="inline-block px-3 py-1 text-white bg-blue-600 rounded-md hover:bg-blue-700">Edit</a>

                            <form action="{{ route('vendor.arrivals.destroy', $item->id) }}" method="POST" class="inline-block"
                ...
                @empty
                    <tr><td colspan="7" class="px-4 py-6 text-center text-slate-400">No arrivals yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
